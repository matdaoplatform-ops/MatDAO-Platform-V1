...
import { analyzeDueDiligence } from "@/lib/ai-studio/due-diligence"
...
Methods
Cells were transfected with plasmids encoding the Cas9 variant and synthetic guide RNAs targeting the BRCA1 locus. Statistical analysis was performed using GUIDE-seq and deep sequencing with appropriate controls.

Results
We achieved 78% on-target editing efficiency with off-target events below 0.3%. Data and supplementary materials are available for reproducibility.`,
  },
  materials: {
    label: "Materials — Perovskite solar cells",
    text: `Abstract
We report a mixed-dimensional perovskite heterostructure that passivates grain boundaries in photovoltaic devices, achieving power conversion efficiency exceeding 24% under AM1.5G illumination with improved ambient stability. This represents a breakthrough in commercial solar applications.

Methods
Perovskite films were deposited via two-step spin coating with a 2D passivation layer applied post-annealing. J-V characteristics and stability testing were performed under controlled humidity at university laboratory facilities.

Results
Champion devices reached 24.2% PCE with negligible hysteresis. Published in peer-reviewed journal.`,
  },
}

export default function DueDiligenceSubmitPage() {
  ...
  const runAnalysis = useCallback(
    async (uploadFile: File) => {
      ...
      setError(null)
      try {
        const report = await analyzeDueDiligence(uploadFile)
        sessionStorage.setItem("matdao-dd-report", JSON.stringify(report))
        router.push("/ai-studio/due-diligence/results")
      } catch (err) {
  ...
  return (
    <div className="relative min-h-[70vh] px-5 py-12 sm:px-6 md:py-16">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-[-10%] h-[700px] w-[700px] -translate-x-1/2 rounded-full bg-amber-300/3 blur-[140px]" />
      </div>

      <div className="workflow-panel relative z-10 mx-auto max-w-3xl rounded-2xl p-6 sm:p-8">
        <p className="mb-2 text-[11px] uppercase tracking-[0.18em] text-[#c5fdff]">9-Dimension Due Diligence</p>
        <h1 className="font-headline mb-2 text-2xl font-bold text-white/95 md:text-3xl">
          Upload Scientific Paper for Scoring
        </h1>
        ...
        >
          {loading ? (
            <>
              ...
              Running 9-dimension analysis...
            </>
          ) : (
            "Run Due Diligence Scoring"
          )}
        </button>
      </div>
    </div>
